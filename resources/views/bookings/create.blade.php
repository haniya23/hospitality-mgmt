...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    <form method="POST" action="{{ route('bookings.store') }}" x-data="bookingCreateForm()" x-init="init()" @submit="handleSubmit" class="space-y-4 sm:space-y-6">
        ...
        <div x-data="{ ...simpleModalScrollLock() }" 
             x-init="setupScrollLock('showPropertySelectionModal')"
             x-show="showPropertySelectionModal" 
             x-transition 
             class="fixed inset-0 overflow-y-auto modal-backdrop" 
    ...
    $('input[name="check_in_date"]').datepicker($.extend({}, datepickerOptions, {
        ...
        onSelect: function(dateText) {
            ...
            setTimeout(() => {
                ...
                const event = new CustomEvent('checkin-date-changed', { 
    ...
    $('input[name="check_out_date"]').datepicker($.extend({}, datepickerOptions, {
        ...
        onSelect: function(dateText) {
            ...
            setTimeout(() => {
                ...
                // Method 3: Dispatch custom event
                const event = new CustomEvent('checkout-date-changed', { 
                    detail: { date: dateText } 
                });
                document.dispatchEvent(event);
            }, 10);
