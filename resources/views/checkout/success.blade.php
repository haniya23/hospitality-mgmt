@extends('layouts.app')

@section('title', 'Check-out Successful')

@section('content')
<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <!-- Success Header -->
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
            <div class="px-6 py-8 text-center">
                <div class="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-green-100 mb-4">
                    <svg class="h-6 w-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                    </svg>
                </div>
                <h1 class="text-2xl font-bold text-gray-900 mb-2">✅ Check-out Successful!</h1>
                <p class="text-gray-600">Guest has been successfully checked out</p>
            </div>
        </div>

        <!-- Check-out Details -->
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-lg font-semibold text-gray-900">Check-out Details</h2>
            </div>
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <h3 class="text-sm font-medium text-gray-500 uppercase tracking-wide mb-3">Guest Information</h3>
                        <dl class="space-y-2">
                            <div>
                                <dt class="text-sm font-medium text-gray-600">Name</dt>
                                <dd class="text-sm text-gray-900">{{ $checkOut->guest_name }}</dd>
                            </div>
                            @if($checkOut->room_number)
                            <div>
                                <dt class="text-sm font-medium text-gray-600">Room Number</dt>
                                <dd class="text-sm text-gray-900">{{ $checkOut->room_number }}</dd>
                            </div>
                            @endif
                            <div>
                                <dt class="text-sm font-medium text-gray-600">Check-out Time</dt>
                                <dd class="text-sm text-gray-900">{{ $checkOut->check_out_time->format('M d, Y \a\t g:i A') }}</dd>
                            </div>
                        </dl>
                    </div>
                    
                    <div>
                        <h3 class="text-sm font-medium text-gray-500 uppercase tracking-wide mb-3">Stay Summary</h3>
                        <dl class="space-y-2">
                            <div>
                                <dt class="text-sm font-medium text-gray-600">Property</dt>
                                <dd class="text-sm text-gray-900">{{ $checkOut->reservation->accommodation->property->name }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-gray-600">Room</dt>
                                <dd class="text-sm text-gray-900">{{ $checkOut->reservation->accommodation->display_name }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-gray-600">Check-in</dt>
                                <dd class="text-sm text-gray-900">{{ $checkOut->checkIn->check_in_time->format('M d, g:i A') }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-gray-600">Duration</dt>
                                <dd class="text-sm text-gray-900">{{ $checkOut->checkIn->check_in_time->diffInDays($checkOut->check_out_time) }} days</dd>
                            </div>
                        </dl>
                    </div>
                </div>
                
                @if($checkOut->services_used && count($checkOut->services_used) > 0)
                <div class="mt-6">
                    <h3 class="text-sm font-medium text-gray-500 uppercase tracking-wide mb-2">Services Used</h3>
                    <div class="flex flex-wrap gap-2">
                        @foreach($checkOut->services_used as $service)
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                            {{ ucfirst(str_replace('_', ' ', $service)) }}
                        </span>
                        @endforeach
                    </div>
                </div>
                @endif
                
                @if($checkOut->service_notes)
                <div class="mt-6">
                    <h3 class="text-sm font-medium text-gray-500 uppercase tracking-wide mb-2">Service Notes</h3>
                    <p class="text-sm text-gray-900">{{ $checkOut->service_notes }}</p>
                </div>
                @endif
            </div>
        </div>

        <!-- Final Settlement -->
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-lg font-semibold text-gray-900">Final Settlement</h2>
            </div>
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div class="text-center">
                        <div class="text-2xl font-bold text-blue-600">₹{{ number_format($checkOut->final_bill, 2) }}</div>
                        <div class="text-sm text-gray-500">Final Bill</div>
                    </div>
                    @if($checkOut->late_checkout_charges > 0)
                    <div class="text-center">
                        <div class="text-2xl font-bold text-orange-600">₹{{ number_format($checkOut->late_checkout_charges, 2) }}</div>
                        <div class="text-sm text-gray-500">Late Check-out</div>
                    </div>
                    @endif
                    @if($checkOut->deposit_refund > 0)
                    <div class="text-center">
                        <div class="text-2xl font-bold text-green-600">₹{{ number_format($checkOut->deposit_refund, 2) }}</div>
                        <div class="text-sm text-gray-500">Deposit Refund</div>
                    </div>
                    @endif
                    <div class="text-center">
                        <div class="text-2xl font-bold text-purple-600">₹{{ number_format($checkOut->net_amount, 2) }}</div>
                        <div class="text-sm text-gray-500">Net Amount</div>
                    </div>
                </div>
                
                <div class="mt-6 text-center">
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium 
                        @if($checkOut->payment_status === 'completed') bg-green-100 text-green-800
                        @elseif($checkOut->payment_status === 'pending') bg-yellow-100 text-yellow-800
                        @elseif($checkOut->payment_status === 'partial') bg-orange-100 text-orange-800
                        @else bg-red-100 text-red-800
                        @endif">
                        Payment Status: {{ ucfirst($checkOut->payment_status) }}
                    </span>
                </div>
                
                @if($checkOut->payment_notes)
                <div class="mt-4 text-center">
                    <p class="text-sm text-gray-600">{{ $checkOut->payment_notes }}</p>
                </div>
                @endif
            </div>
        </div>

        <!-- Feedback -->
        @if($checkOut->rating || $checkOut->feedback_comments)
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-lg font-semibold text-gray-900">Guest Feedback</h2>
            </div>
            <div class="p-6">
                @if($checkOut->rating)
                <div class="flex items-center mb-4">
                    <span class="text-sm font-medium text-gray-600 mr-3">Rating:</span>
                    <div class="flex">
                        @for($i = 1; $i <= 5; $i++)
                        <svg class="w-5 h-5 {{ $i <= $checkOut->rating ? 'text-yellow-400' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20">
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
                        </svg>
                        @endfor
                    </div>
                </div>
                @endif
                
                @if($checkOut->feedback_comments)
                <div>
                    <h3 class="text-sm font-medium text-gray-600 mb-2">Comments:</h3>
                    <p class="text-sm text-gray-900">{{ $checkOut->feedback_comments }}</p>
                </div>
                @endif
            </div>
        </div>
        @endif

        <!-- Room Status -->
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-lg font-semibold text-gray-900">Room Status</h2>
            </div>
            <div class="p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-sm font-medium text-gray-600">Room Cleaning Status</h3>
                        <p class="text-sm text-gray-900">
                            @if($checkOut->room_marked_clean)
                                ✅ Room marked as clean and ready for next guest
                            @else
                                ⏳ Room needs cleaning
                            @endif
                        </p>
                    </div>
                    @if(!$checkOut->room_marked_clean)
                    <form action="{{ route('checkout.mark-clean', $checkOut->uuid) }}" method="POST" class="inline">
                        @csrf
                        <button type="submit" 
                                class="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2">
                            Mark Room Clean
                        </button>
                    </form>
                    @endif
                </div>
            </div>
        </div>

        <!-- Actions -->
        <div class="flex justify-center space-x-4">
            <a href="{{ route('bookings.show', $checkOut->reservation->uuid) }}" 
               class="px-6 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-500">
                View Booking Details
            </a>
            <a href="{{ route('bookings.index') }}" 
               class="px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
                Back to Bookings
            </a>
        </div>
    </div>
</div>
@endsection
