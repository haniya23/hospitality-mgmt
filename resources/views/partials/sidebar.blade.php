    ...
    <div class="sidebar-desktop hidden lg:fixed lg:top-16 lg:bottom-0 lg:left-0 lg:z-40 lg:bg-white lg:shadow-md lg:shadow-green-200/50 lg:flex lg:flex-col lg:overflow-y-auto transition-all duration-300"
        ...
        <template x-if="sidebarCollapsed">
            <div class="flex flex-col items-center justify-center h-full py-4 space-y-4">
                ...
                <div class="relative w-12 h-12" x-data="{ 
                    ...
                    <svg class="w-12 h-12 transform -rotate-90" viewBox="0 0 36 36">
                        <path stroke="#e5e7eb" stroke-width="3" fill="transparent"
                            d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"></path>
                        <!-- Progress Circle -->
                        <path stroke="{{ $strokeColor }}" stroke-width="3" fill="transparent" stroke-linecap="round"
                    ...
                    <div class="mb-6">
                        ...
                        <ul class="space-y-1">
                            <li><a href="{{ route('subscription.plans') }}"
                                    class="flex gap-4 p-3 font-semibold rounded-lg hover:bg-green-100 {{ request()->routeIs('subscription.*') ? 'bg-gradient-to-r from-green-400 to-green-600 text-white' : 'text-gray-700' }} transition-all">
                ...
                <div class="mt-4 p-4 bg-gradient-to-r {{ $bgGradient }} rounded-xl border {{ $borderColor }}">
                    <div class="flex items-center justify-between mb-3">
                        <div class="flex items-center gap-3">
                            ...
                            <!-- Battery Indicator -->
                            <div class="relative w-8 h-8" x-data="{ 
                                     percentage: {{ $percentage }}, 
                                     animatedPercentage: 0,
                                     init() {
                                         setTimeout(() => {
                                ...
                                <svg class="w-8 h-8 transform -rotate-90" viewBox="0 0 36 36">
                                    <path stroke="#e5e7eb" stroke-width="4" fill="transparent"
                                        d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831">
                                    </path>
                                    <!-- Progress Circle -->
                                    <path stroke="{{ $strokeColor }}" stroke-width="4" fill="transparent"
                                ...
                                <div class="absolute inset-0 flex items-center justify-center">
                                    <span class="text-xs font-bold text-gray-700"
                                        x-text="Math.round(animatedPercentage) + '%'"></span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="flex items-center justify-between text-sm text-gray-600">
                        <div>
                            ...
                            @else
                                {{ auth()->user()->properties_limit }}
                                {{ Str::plural('property', auth()->user()->properties_limit) }}
                            @endif
                        </div>
                        ...
                        <div class="flex items-center gap-2">
                            <div class="w-16 h-2 bg-gray-200 rounded-full overflow-hidden">
                                <div class="h-full transition-all duration-2000 ease-out rounded-full"
                                    ...
                                    x-data="{ width: 0 }" x-init="setTimeout(() => { width = {{ $percentage }} }, 500)"
                                    :style="'width: ' + width + '%'">
                                </div>
                            </div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="relative w-8 h-8" x-data="{ 
                        ...
                        <svg class="w-8 h-8 transform -rotate-90" viewBox="0 0 36 36">
                            ...
                            <!-- Progress Circle -->
                            <path stroke="{{ $strokeColor }}" stroke-width="4" fill="transparent" stroke-linecap="round"
                                class="transition-all duration-2000 ease-out"
                                :stroke-dasharray="animatedPercentage + ' 100'"
                                d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831">
                            </path>
            ...
            <div class="flex items-center justify-between text-sm text-gray-600">
                ...
                <div class="flex items-center gap-2">
                    <div class="w-16 h-2 bg-gray-200 rounded-full overflow-hidden">
                        <div class="h-full transition-all duration-2000 ease-out rounded-full"
                            style="background-color: {{ $strokeColor }}; width: {{ $percentage }}%"
                            x-data="{ width: 0 }" x-init="setTimeout(() => { width = {{ $percentage }} }, 600)"
                            :style="'width: ' + width + '%'">
                        </div>
                    </div>
                    <span class="text-xs font-medium text-gray-500">{{ round($percentage) }}%</span>
