...
<div class="bg-white rounded-2xl shadow-sm p-6 mb-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Edit Accommodation</h1>
            <p class="text-gray-600">Update accommodation details</p>
        </div>
...
@section('content')
<div class="space-y-6">
    <form method="POST" action="{{ route('accommodations.update', $accommodation) }}" x-data="accommodationForm()" x-init="init()" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="property_id" id="property_id" required
                        ...
                        @foreach($properties as $property)
                            <option value="{{ $property->id }}" {{ old('property_id', $accommodation->property_id) == $property->id ? 'selected' : '' }}>
                                {{ $property->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="custom_name" id="custom_name" required
                           ...
                           value="{{ old('custom_name', $accommodation->custom_name) }}">
                    @error('custom_name')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="predefined_type_id" id="predefined_type_id"
                        ...
                        @foreach($predefinedTypes as $type)
                            <option value="{{ $type->id }}" {{ old('predefined_type_id', $accommodation->predefined_type_id) == $type->id ? 'selected' : '' }}>
                                {{ $type->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="base_price" id="base_price" required min="0" step="0.01"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                           value="{{ old('base_price', $accommodation->base_price) }}">
                    @error('base_price')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mt-6">
                ...
                <textarea name="description" id="description" rows="4"
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                          placeholder="Describe the accommodation...">{{ old('description', $accommodation->description) }}</textarea>
            </div>
        </div>
                ...
                @foreach($amenities as $amenity)
                    <label class="flex items-center space-x-2 cursor-pointer">
                        <input type="checkbox" name="amenities[]" value="{{ $amenity->id }}"
                               ...
                               {{ in_array($amenity->id, old('amenities', $accommodation->amenities->pluck('id')->toArray())) ? 'checked' : '' }}>
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6">
            <h2 class="text-xl font-semibold text-gray-900 mb-4">Current Photos</h2>
        ...
        <!-- New Photos -->
        <div class="bg-white rounded-2xl shadow-sm p-6">
            <h2 class="text-xl font-semibold text-gray-900 mb-4">Add New Photos</h2>
