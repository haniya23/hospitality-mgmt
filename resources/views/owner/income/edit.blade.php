...
@section('content')
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <form method="POST" action="{{ route('owner.income.update', $income) }}" class="space-y-4 sm:space-y-6">
            @csrf @method('PUT')
                    ...
                    <div>
                        ...
                        <select name="property_id" id="property_id" required
                            ...
                            @foreach($properties as $property)
                                <option value="{{ $property->id }}" {{ $income->property_id == $property->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="income_type" id="income_type" required
                            ...
                            @foreach($incomeTypes as $value => $label)
                                <option value="{{ $value }}" {{ $income->income_type == $value ? 'selected' : '' }}>{{ $label }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
                    ...
                    <div>
                        ...
                        <input type="date" name="transaction_date" id="transaction_date" required
                            value="{{ $income->transaction_date->format('Y-m-d') }}"
                            class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 bg-white hover:border-gray-300 font-semibold text-gray-800">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5 mt-5">
                    <div>
                        ...
                        <select name="payment_status" id="payment_status" required
                            ...
                            <option value="unpaid" {{ $income->payment_status == 'unpaid' ? 'selected' : '' }}>Unpaid</option>
                            <option value="partial" {{ $income->payment_status == 'partial' ? 'selected' : '' }}>Partial
                            </option>
                        </select>
                    </div>
                    <div>
            ...
            <div
                ...
                <div class="mb-5">
                    ...
                    <input type="text" name="reference_number" id="reference_number" value="{{ $income->reference_number }}"
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3"
                        class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all duration-200 bg-white hover:border-gray-300 font-semibold text-gray-800">{{ $income->notes }}</textarea>
                </div>
            </div>
            ...
            <div class="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-4 pb-20 lg:pb-8">
                ...
                <button type="submit"
                    class="w-full sm:w-auto px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-xl hover:from-green-600 hover:to-emerald-700 transition-all shadow-lg font-semibold">Update</button>
            </div>
        </form>
    </div>
@endsection